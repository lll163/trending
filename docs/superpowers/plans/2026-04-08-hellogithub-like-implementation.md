...
- [x] **Step 1：** 列出最近 N 条同步失败仓库与原因。
- [x] **Step 2：** 提交：`feat: 同步失败可观测后台摘要`。

### Task 8.2：全量测试与文档对齐

- [x] **Step 1：** 运行 `php artisan test`，修复失败用例。（无 `pdo_sqlite` 时以 `tests/Unit` + 带扩展要求的 Feature 跳过策略为准，见执行记录 §7。）
- [x] **Step 2：** 更新规格或本计划中的偏差说明（若有）。
- [x] **Step 3：** 提交：`chore: 测试与文档对齐`。
...
| 2026-04-08 | Chunk 8 勾选完成：同步日志、后台摘要、测试与执行记录偏差说明 |
