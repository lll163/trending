                    ...
                    <form method="post" action="{{ route('admin.articles.update', $article) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="magazine_issue_id" name="magazine_issue_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                ...
                                @foreach ($issues as $iss)
                                    <option value="{{ $iss->id }}" @selected(old('magazine_issue_id', $article->magazine_issue_id) == $iss->id)>
                        ...
                        <div>
                            ...
                            <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full font-mono"
                                :value="old('slug', $article->slug)" required autocomplete="off" />
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full"
                                :value="old('title', $article->title)" required autocomplete="off" />
                        ...
                        <div>
                            ...
                            <textarea id="body" name="body" rows="18" required
                                class="mt-1 block w-full border-gray-300 font-mono text-sm rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('body', $article->body) }}</textarea>
                            <x-input-error class="mt-2" :messages="$errors->get('body')" />
                        </div>

                        <div>
                            <x-input-label for="sort_order" :value="__('排序')" />
                            <x-text-input id="sort_order" name="sort_order" type="number" min="0" class="mt-1 block w-32"
                                :value="old('sort_order', $article->sort_order)" required />
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                <option value="draft" @selected(old('status', $article->status) === 'draft')>{{ __('草稿') }}</option>
                                <option value="published" @selected(old('status', $article->status) === 'published')>{{ __('已发布') }}</option>
                            </select>
